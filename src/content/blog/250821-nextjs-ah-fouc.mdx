...
light theme so the user got **💣 flashbanged 💣**. If you add some styles to
...
GitHub Pages and I can't use any HTTP APIs. 😅 So, we're left with solution #2.
...
- ```tsx
  ...
  is what we pass into our `ThemeContext`.

- The `applyTheme` callback actually updates the theme in both `localStorage`
...
- ```tsx
  ...
  This `useEffect` sets the initial value for `themeState`.
...
- ```tsx
  ...
  from before as an
  [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) and places it
  in a `<script>` element just above our app content which makes it run first.
...
The solution above satisfies my use case, but there are definitely a few code
smells, notably:
