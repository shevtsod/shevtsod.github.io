...
I recently ran into FOUC while working on my personal website. Here's what
happened:
...
```tsx:components/layout/layout.tsx showLineNumbers {7}
'use client';
...
```js:components/theme-provider/script.js
function script() {
  try {
    const storedTheme = localStorage.getItem('theme');
    const preferredTheme = window.matchMedia('(prefers-color-scheme: dark)')
...
```tsx:components/theme-provider/theme-provider.tsx
...
> client, i.e.,
>
> ```tsx:components/my-component/my-component.tsx
> 'use client';
>
...
The solution above satisfies my use case but is definitely hacky in some places,
notably:
